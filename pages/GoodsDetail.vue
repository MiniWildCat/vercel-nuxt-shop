    ...
    <div class="shop-item-path">
      <div class="shop-nav-container">
        <Breadcrumb>
          ...
          <BreadcrumbItem v-for="(item, index) in categoryBar" :to="goGoodsList(index)" target="_self" :key="index">
            {{ item.name }}
          </BreadcrumbItem>
        </Breadcrumb>
...
import { goodsSkuDetail,goodsSkuDetailSEO } from "@/api/goods";
import {
  cancelStoreCollect,
  collectStore,
  isStoreCollection,
...
export default {
  ...
  // SEO开始
  async fetch(){
    await this.getGoodsSkuDetail(this.$route.query)
  },
  head(){
    return {
      title: this.goodsMsg?.data?.goodsName,
      meta: [
        {hid: 'description',name:'description',content: this.goodsMsg?.data?.sellingPoint },
        {hid: 'keywords',name:'keywords',content: this.categoryBar.map(item=>item.name).join('、')}
      ]
    }
  },
  // SEO结束
  ...
  methods: {
    ...
    targetClickSku (val) {
      ...
      this.getGoodsSkuDetail(val,false)
    },
    // 获取商品详情
    getGoodsDetail (val) {
      this.isLoading = true;
      ...
      let distributionId =
        params && params.distributionId
          ...
          : storage.getItem("distributionId");
      // 如果有分销信息
      if (distributionId) {
        console.log(distributionId);
        // 先存储
        storage.setItem("distributionId", params.distributionId);
        let _this = this;
        // 绑定关系
        getGoodsDistribution(params.distributionId).then((res) => {
          // 绑定成功，则清除关系
          if (res.success) {
            storage.removeItem("distributionId");
          }
        });
      }
      this.isLoading = false;
    },
    getGoodsSkuDetail(params,isInit=true){
      return ( isInit ? goodsSkuDetailSEO(this.$route.query) : goodsSkuDetail(params))
        .then((res) => {
          this.isLoading = false;
          if (res.success) {
