...
import { getCategorySEO } from "@/api/goods";
export default {
  name: 'GoodsClassNav',
  data () {
    return {
      ...
      params: {}, // 请求参数
      // SEO 信息
      metaInfo: {title:'',keywords:'',description:''},
      cateListSEO: []
    };
  },
  computed: {
  ...
  methods: {
    getCate(){
      return getCategorySEO(0).then((res) => {
        if (res.success) {
          this.cateListSEO = res.result;
        }
      });
    },
    async getNav (isInit=true) { // 获取商品分类，分类下展示
      console.log("1",this.cateListSEO)
      ...
      console.log("2")
      if(isInit){
        if (!this.cateListSEO.length) { // 商品分类存储在localstorage，接口未调用成功前再次刷新数据
          setTimeout(async () => {
            await this.getNav(isInit)
          }, 500)
          return
        }
      }else{
      ...
      if (arr.length > 0) {
        this.tabBar = (isInit ? this.cateListSEO : this.cateList).filter(e => {
          return e.id === arr[0]
        })[0]
        keywords.push(this.tabBar.name);
      }
      if (arr.length > 1) {
      ...
      this.metaInfo = {
        ...
        description: "买"+keywords[0]+"就到lilishop商城"
      }
    },
